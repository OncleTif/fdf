...
/*   Updated: 2016/02/04 14:01:44 by tmanet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_projector(t_graph *grp, t_node *node)
{
	...
	node->p_x = (node->x * x_factor + node->y * y_factor - node->z * z_factor) * grp->zoom + grp->x_offset;
	node->p_y = (node->x * x_factor - node->z * z_factor - y_factor * node->y) * grp->zoom + grp->y_offset;
			}
